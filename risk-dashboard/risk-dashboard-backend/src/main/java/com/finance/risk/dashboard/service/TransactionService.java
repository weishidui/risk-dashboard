...
public interface TransactionService {
     ...
     * 统计近一段时间内活跃用户数（去重）
     */
    Long countDistinctUsers(Long startTime, Long endTime);

    /**
     * 平均点击耗时（延迟代理指标）
     */
    Double avgClickDuration(Long startTime, Long endTime);

    /**
     * 按设备安全分阈值统计通过数（dev_score >= 80）
     */
    Long countPassByDevScore(Long startTime, Long endTime);

    /**
     * 按设备安全分阈值统计拦截数（dev_score < 50）
     */
    Long countBlockByDevScore(Long startTime, Long endTime);
}
