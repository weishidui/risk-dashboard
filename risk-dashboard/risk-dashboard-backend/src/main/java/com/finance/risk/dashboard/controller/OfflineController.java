...
public class OfflineController {
    ...
    public Result<Map<String, Object>> startAnalyze(@RequestBody(required = false) Map<String, String> body) {
        String dt = body != null ? body.get("dt") : null;
        Map<String, Object> result = analysisService.startRecent30DaysAnalysis(dt);
    ...
    public Result<Map<String, Object>> analyzeStatus(@RequestParam(defaultValue = "") String dt) {
        if (dt.isEmpty()) {
            dt = java.time.LocalDate.now().format(java.time.format.DateTimeFormatter.ofPattern("yyyyMMdd"));
        }
