    ...
    <div v-else class="app-shell">
      ...
      <header class="topbar">
        <div class="topbar-left">
          <!-- 模式切换 -->
          <div class="mode-tabs">
            <span class="mode-tab" :class="{ active: mode === 'realtime' }" @click="switchMode('realtime')">
              <span class="mode-dot" :class="{ live: mode === 'realtime' }"></span>实时监控
            </span>
            <span class="mode-tab" :class="{ active: mode === 'offline' }" @click="switchMode('offline')">
              <i class="el-icon-s-data"></i> 离线分析
            </span>
          </div>
        </div>
        <div class="topbar-right">
          <span class="health-dots" @click="showHealthDetail = true" style="cursor:pointer;" title="点击查看链路详情">
