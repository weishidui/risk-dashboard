...
`t_metrics` 不在应用启动时生成模拟快照。历史指标只由实时链路写入 Redis 后，经后端定时任务持久化。
...
**错误日志：**
```
实时指标快照写入失败，请检查 `t_metrics` 表和 Redis 实时指标桶。
```

这说明实时指标定时快照无法写入。请先建表并检查 Redis 连接，然后等待下一次一分钟定时任务。
...
```bash
# 1. 确认表存在
mysql -h 192.168.125.104 -u root -p123456 -e "SHOW TABLES" risk_control | grep t_metrics

# 2. 确认 sys_user 有 admin 账号
mysql -h 192.168.125.104 -u root -p123456 -e "SELECT * FROM sys_user WHERE username='admin'" risk_control
...
# 5. 等 2 分钟后检查快照是否产生
sleep 120
mysql -h 192.168.125.104 -u root -p123456 -e "SELECT COUNT(*) FROM t_metrics" risk_control
```
